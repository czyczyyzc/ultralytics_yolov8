...
2026-09-18 扩展说明：现已准备 37 视频真实数据快照，并启动覆盖这些训练来源的新合成批次；补齐了 6 个旧标注来源及 4 个历史图片目录的哈希映射。新任务支持跨批次排除、分进程生成和完成后校验。本文以下的 856 张、28 视频及运行命令保留为旧批次复现说明；最新路径、数量口径和运行状态见 `../gray_expansion_20260918/README.md`，不要把新任务的最大尝试数量当成已生成数量。
