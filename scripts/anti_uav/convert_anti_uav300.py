...
def discover_sequences(source_root: Path) -> list[dict]:
    """Find Anti-UAV sequence directories across legacy and train-only layouts."""
    sequences = {}
...
def find_video_for_modality(sequence_dir: Path, modality: str, label_stem: str | None = None) -> Path | None:
    """Find the matching video file for a modality across legacy and train-only layouts."""
    candidate_stems = []
    if label_stem:
