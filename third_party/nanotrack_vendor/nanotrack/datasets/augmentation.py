...
from nanotrack.utils.bbox import Center, Corner, center2corner, corner2center
...
class Augmentation:
    def __init__(self, shift, scale, blur, flip, color):
        ...
        self.rgbVar = np.array(
            [[-0.55919361, 0.98062831, -0.41940627], [1.72091413, 0.19879334, -1.82968581], [4.64467907, 4.73710203, 4.88324118]],
            dtype=np.float32,
        )
    ...
    def _crop_roi(self, image, bbox, out_sz, padding=(0, 0, 0)):
        ...
        return cv2.warpAffine(image, mapping, (out_sz, out_sz), borderMode=cv2.BORDER_CONSTANT, borderValue=padding)
    ...
    def _blur_aug(self, image):
        ...
        center = size // 2
        wx = np.random.random()
        kernel[:, center] += 1.0 / size * wx
        kernel[center, :] += 1.0 / size * (1 - wx)
        return cv2.filter2D(image, -1, kernel)

    def _color_aug(self, image):
        offset = np.dot(self.rgbVar, np.random.randn(3, 1))[::-1].reshape(3)
        return image - offset

    @staticmethod
    def _gray_aug(image):
        grayed = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        return cv2.cvtColor(grayed, cv2.COLOR_GRAY2BGR)
    ...
    def _shift_scale_aug(self, image, bbox, crop_bbox, size):
        ...
        if self.scale:
            ...
            crop_bbox_center = Center(crop_bbox_center.x, crop_bbox_center.y, crop_bbox_center.w * scale_x, crop_bbox_center.h * scale_y)
        ...
        return self._crop_roi(image, crop_bbox, size), bbox
